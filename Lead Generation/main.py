...
serper_api_keys = [
    "92f3dc3a0816a03bd6a95290fe1a7779ffebf1a1",  # replace with your actual key
    "b3520cb683497d6ec67bfbb366e4038455bc40af",
    "7c4f1baaa12d2e468c744d56db4723be0eaaf04e"
]
...
def search_with_api_key(query, api_key, engine='google'):
    ...
    elif api_key == api_keys[2]:  # SerperAPI
        for serper_key in serper_api_keys:
            url = "https://google.serper.dev/search"
            headers = {
            ...
            if response.status_code == 200:
                ...
                print(f"SerperAPI key {serper_key} failed with status code {response.status_code}")
...
# Tavily API keys for LinkedIn URL extraction
tavily_api_keys = [
    "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",  # replace with your actual key
    "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",
    "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",
    "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"
]

# Tavily API for LinkedIn URL extraction with fallback
def extract_linkedin_urls_with_tavily(query):
    for tavily_key in tavily_api_keys:
        tavily_client = TavilyClient(api_key=tavily_key)
        try:
            response = tavily_client.search(query)
            # If the response contains results, return them
            if 'results' in response:
                print(f"Using Tavily API key {tavily_key} for query: {query}")
                return response['results']
            else:
                print(f"No LinkedIn URLs found in Tavily response for query: {query}")
        except Exception as e:
            print(f"Tavily API key {tavily_key} failed with error: {e}")

    # If all Tavily keys fail, return an empty list
    print("All Tavily API keys failed.")
    return []
